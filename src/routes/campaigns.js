...
campaigns.post('/', async (req, res) => {
  const { platform, niche, location, keywords, maxLeads = 25 } = req.body;
  if (!platform) return res.status(400).json({ error: 'platform requerida' });

  const campaignId = db.prepare(
    'INSERT INTO campaigns (platform, niche, location, keywords, max_leads, status) VALUES (?,?,?,?,?,?)'
  ).run(platform, niche, location, keywords, maxLeads, 'running').lastInsertRowid;
